...
router.get('/report', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    workbook.creator = 'IT Centrale Management System';
    ...
    coverSheet.addRow(['']);
    coverSheet.addRow(['', 'Generated by IT Centrale Management System']);
    ...
    const filename = `IT-Centrale-Report-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.xlsx`;
